...
namespace DTcms.Web.UI.Page
{
    public partial class search : Web.UI.BasePage
    {
        ...
        protected DataTable get_search_list(int _pagesize, out int _totalcount)
        {
            ...
            if (site.parent_id > 0)
            {
                _siteid = site.parent_id;
            }
            if (!string.IsNullOrEmpty(tags))
            {
                ds = new BLL.article().ArticleSearch(_siteid, tags, _pagesize, page, string.Empty, "add_time desc,id desc", out _totalcount);
            }
            else
            {
                ds = new BLL.article().ArticleSearch(_siteid, _pagesize, page, "(title like '%" + keyword + "%' or zhaiyao like '%" + keyword + "%')", "add_time desc,id desc", out _totalcount);
            }
